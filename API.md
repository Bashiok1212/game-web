...
| `versionNumber` | 版本编号（可选，该张卡在扩展内的编号，如 `078/207`、卡序号） |
| `rarity` | 稀有度 |
...
独立页：`/ptcg-fields.html`。为 `language`、`rarity`、`condition`、`cardStatus` 配置选项（每行一项）；**版本**支持 **`versionByLanguage`**：每种语言对应一个数组，每项可为**字符串**（仅名称）或对象 **`{ "name": "朱紫", "year": 2023, "code": "SV1", "maxNo": 207 }`**（`year` / `code` / **`maxNo`（编号上限，1～999999）** 可选）。**数组顺序**即为录入页该语言下「版本」下拉的顺序（编辑页可对版本行、语言块使用 ↑↓ 调整）。**`language`** 字符串数组的顺序与语言块顺序一致，用于重新打开编辑页时恢复语言行顺序。另保留 **`version`** 作**全局兜底**（格式与上相同，未配置 `versionByLanguage` 或旧数据时使用）。录入页从下拉选版本时，可根据配置自动填充卡牌的年份与 `versionCode`；编号上限会显示在下拉项文案中。某键**未保存过**时，合并接口对该键返回空数组（卡状态除外）；**保存为空数组**表示该字段在录入页用手输。
