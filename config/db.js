...
mongoose.connection.on('connected', async () => {
  ...
  // 移除 playeritems 的 character_1_item_1 唯一索引（堆叠上限 99 后允许多格同物品）
  try {
    ...
    if (uniqueIdx) {
      ...
      console.log('已删除 playeritems character_1_item_1 唯一索引');
    }
  } catch (e) {
    if (e.code !== 27 && e.codeName !== 'IndexNotFound') console.error('drop character_1_item_1:', e.message);
  }
});
